...
(function () {
  ...
  function fillContactForm() {
    const fields = {
      'input[type="text"]:nth-of-type(1)': contactData.name,
      'input[type="email"]':               contactData.email,
      'input[type="text"]:nth-of-type(2)': contactData.subject,
      'textarea':                          contactData.message,
    };
    // More robust: target by placeholder
    const form = document.querySelector('.contact-inner');
    if (!form) return;
    const inputs = form.querySelectorAll('input, textarea');
    const order  = ['name', 'email', 'subject', 'message'];
    inputs.forEach((el, i) => {
      if (order[i] && contactData[order[i]]) {
        el.value = contactData[order[i]];
  ...
  function processCommand(transcript) {
    ...
    // ── Theme ──
    if (/dark mode|dark theme|डार्क मोड/.test(t))                 { document.documentElement.setAttribute('data-theme','dark'); document.querySelector('.theme-icon').textContent='☀️'; localStorage.setItem('theme','dark'); addPopupEntry('assistant','Dark mode enabled.'); speak('Dark mode enabled.'); return; }
    if (/light mode|light theme|लाइट मोड/.test(t))                { document.documentElement.removeAttribute('data-theme'); document.querySelector('.theme-icon').textContent='🌙'; localStorage.setItem('theme','light'); addPopupEntry('assistant','Light mode enabled.'); speak('Light mode enabled.'); return; }
    if (/toggle (theme|mode)|थीम बदलो/.test(t))                   { document.getElementById('themeToggle')?.click(); addPopupEntry('assistant','Theme toggled.'); speak('Theme toggled.'); return; }
